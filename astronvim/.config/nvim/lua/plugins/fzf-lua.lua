  ...
  dependencies = {
    {
      ...
      opts = function(_, opts)
        ...
        maps.n["<Leader>ff"] = {
          function() require("fzf-lua").files { prompt = "LS> ", cwd = require("astrocore.rooter").root() } end,
          desc = "Find files",
        }
        ...
        if vim.fn.executable "rg" == 1 or vim.fn.executable "grep" == 1 then
          maps.n["<Leader>fw"] = {
            function() require("fzf-lua").live_grep_native { cwd = require("astrocore.rooter").root() } end,
            desc = "Find words",
          }
        end
