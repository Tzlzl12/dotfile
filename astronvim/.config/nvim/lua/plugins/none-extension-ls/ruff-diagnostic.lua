...
local function nested_entry_getter(key1, key2)
  return function(entries, line) return entries[key1][key2] end
end

local function ruff_severity(entries, line)
  local code = entries["code"]
  if code ~= nil then return code_to_severity[string.sub(code, 1, 1)] or severity_default end
...
return h.make_builtin {
  ...
  generator_opts = {
    ...
    format = "json",
    check_exit_code = function(code) return code == 0 end,
    to_stdin = true,
    ignore_stderr = true,
