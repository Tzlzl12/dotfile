...
return {
  ...
  },
  {
      ...
      vim.api.nvim_create_autocmd("BufEnter", {
        ...
        callback = function()
          ...
          if state and state.type == "directory" then
            vim.g.workspace = vim.fn.argv(0)
            local buf = vim.api.nvim_get_current_buf()
            vim.api.nvim_buf_delete(buf, { force = true })
            Snacks.picker.explorer { cwd = vim.g.workspace }
          end
        end,
      })
    end,
    ...
    keys = {
      { "<c-n>", function()
        Snacks.picker.explorer({ cwd = require("utils").workspace() })
      ...
      { "<leader>fq", function() Snacks.picker.qflist({ preset = "ivy" }) end,                                    desc = "Snakcs Quickfix List" },
      {
        ...
        function()
          -- snacks.lua 调用处
          require("utils.select-themes").colorscheme()
          -- Snacks.picker.colorschemes(require("utils.select-themes"))
          -- Snacks.picker(require("utils.select-themes"))
        end,
